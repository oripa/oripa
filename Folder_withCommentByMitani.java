...
public class Folder {
    ...
    public int fold() {
        ...
        // Set overlap relations, which is represented by a matrix, based on valley/mountain folds information
        // Determines the overlap relations which are explicitly decided only by the fold-edges
        // i.e. the relations between neighboring face pairs 
        step1();

        // construct constraint sets
        // these constraints reduce the number of combinations that the system has to check later.
        // the constraints are stored on each face
        
        // there exists two types of constraints
        // one is decided between three faces, the other is decided between four faces
        
        // construct three faces constraint
        // this is to avoid the case of a face penetrates a fold edge, see following fig (b)
        // http://mitani.cs.tsukuba.ac.jp/origami/img/invalid_cases.png
        // If face[i] and face[j] touching edge is covered by face[k]
        // then OR[i][k] = OR[j][k]; this is a constrain for faces{i, j, k} 
        // the exact value (UPPER, LOWER) cannot be decided yet. 
        // therefore, simply holds the information now
        holdCondition3s();
        
        // construct four faces constraints
        // when two fold-edges exist at the same position, a special care is needed 
        // there are four faces related to this situation
        // see following fig (c)
        // http://mitani.cs.tsukuba.ac.jp/origami/img/invalid_cases.png
        
        // assume that face A and B, face C and D are connected respectively,
        // assume that A is upper than B, and C is upper than D
        // then, there exit four possible cases of over-lap order.
        // i.e. A>B>C>D, C>D>A>B, A>C>D>B, C>A>B>D
        // however, it cannot be decided yet. it should be decided later.
        // so now, the system simply holds the info of face index of four faces.
        // i.e. A(upper1),B(lower1) and C(upper2),D(lower2)
        holdCondition4s();

        // fill blank cells of the matrix (ORmat), which are decided by the constraints
        estimation(m_doc.overlapRelation);
        
        // since all cells which can be decided by known info are decided,
        // we search valid matrices by filling remaining blank cells
        ...
        // find overlap order of faces in each subface
    ...
    // Determines the overlap relations which are decided only by the fold-edges
    // i.e. the relations between neighboring pairs 
    private void step1() {
        for (OriFace face : m_doc.faces) {
            for (OriHalfedge he : face.halfedges) {
                if (he.pair == null) {
