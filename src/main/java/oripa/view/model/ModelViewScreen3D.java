...
class J3DFace {
	...
	J3DFace(final OriFace f) {
		oriFace = f;
		bSelected = false;
	}
}

public class ModelViewScreen3D extends Canvas3D implements MouseListener, MouseMotionListener,
		MouseWheelListener,
		ComponentListener {
	...
	public ModelViewScreen3D(final GraphicsConfiguration config) {
		super(config);
	...
	public void setModel(final EstimationEntityHolder document) {
		...
		picker.setupCallback(new SimplePickingCallback() {
			...
			@Override
			public void picked(final int type, final Node node) {
				if (node == null) {
					return;
				}
	...
	private void setSelected(final J3DFace face) {
		for (J3DFace f : faces) {
	...
	private void setColor(final Shape3D sp, final Color3f color) {
...
class SimplePicking extends PickMouseBehavior {
	...
	public SimplePicking(final BranchGroup root, final Canvas3D canvas, final Bounds bounds) {
	...
	public void setupCallback(final SimplePickingCallback callback) {
